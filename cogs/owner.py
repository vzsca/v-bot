    ...
    async def owner_list(self, ctx):
        ...
        for uid in authorized_ids:
            if uid in config.PERMANENT_OWNERS_SET:
                ...
                remaining = max(0, int(state.temp_authorized_users.get(uid, 0) - now))
                description += f"⏳ <@{uid}> ({remaining} seconds remaining)\n"
        embed = discord.Embed(title="📋 Authorized Users", description=description, color=discord.Color.green())
