"""
Sensitive commands: raid, remove_raid, dmall, spam.

This cog is intentionally NOT always loaded. main.py only adds it to the bot
if config.DANGEROUS_COMMANDS_ENABLED is true (read from .env). When disabled
(by default), these commands simply do not exist in the bot's command tree —
they are not just blocked by a check; they are completely absent and cannot
be invoked or discovered.

The toggle is enabled/disabled from the start_bot.bat panel
(toggle_dangerous command), not through a Discord command, and requires a
bot restart for the change to take effect because the choice of cogs to load
is made only once when main.py starts.
"""
...
class DangerousCog(commands.Cog, name="Sensitive"):
    ...
    async def spam(self, ctx, times: int, *, message: str):
        """Controlled message sending — limited and protected."""
        if times < 1 or times > config.MAX_SPAM:
            await ctx.send(f"Invalid amount: times must be between 1 and {config.MAX_SPAM}.")
            return
        try:
    ...
    async def dmall(self, ctx, *, message: str):
        """DM all members (danger: potential spam). Protections: delay and checks."""
        await ctx.send("📨 Sending messages (controlled operation).")
    ...
    async def raid(self, ctx, amount: int = 10):
        ...
        try:
            ...
            await ctx.send(
                ...
                f"🧹 Use `v!remove_raid` to clean up"
            )
    ...
    async def remove_raid(self, ctx):
        """Fast and safe cleanup of elements created by raid."""
        deleted_channels = 0
        deleted_roles = 0
        deleted_messages = 0
